...
public class AuthenDaoHibernate extends AbstractHibernateStorageDao implements
	AuthenDao {
	...
	public Collection<?> find( int pageNumber ) {
		List<Authen> result = new ArrayList<Authen>();

		String queryString = "select authen FROM Authen AS authen ORDER BY authen.uniqueId DESC";
	...
	public Authen findByPartyId( Integer id ) {
		...
		try {
			String queryString = "select new list(authen) " + "FROM Authen AS authen "
