...
public abstract class UploadServlet extends DispatchServlet {
	...
	protected abstract void
			addImage( HttpServletRequest request, FileItem item );

	protected abstract String getServletPath();

	public	void
			htmlGetForm(	FormBean form,
		...
		if ( request.getParameter( "getfile" ) != null && !request.getParameter( "getfile" )
						.isEmpty() ) {
			File file = new File( getFile( request ),
		...
		try {
			...
			for ( FileItem item : items ) {
				if ( !item.isFormField() ) {
					...
					jsono.put( "url",
							getServletPath() + "?getfile=" + item.getName() );
					jsono.put( "thumbnail_url", getServletPath() + "?getthumb="
												+ item.getName() );
					jsono.put( "delete_url", getServletPath() + "?delfile="
												+ item.getName() );
