...
<title>${ktm:getText("page.gallery.title")}</title>
				...
				<div class="row-fluid">
						...
						<div class="span9  ">
								...
								<ul class="breadcrumb">
										...
										<li><a
												href="CRUDGallery?method=list&module=gallery&pageNumber=0"
										>${ktm:getText('page.gallery.title') }</a> <span class="divider">/</span></li>
										<li class="active">${ktm:getText('page.gallery.edit') }</li>
								</ul>
								<br />
								<!--  ===========form====================-->
								<form id="frmContent" method="post" action="CRUDGallery"
								...
								>
										...
										<div class="control-group">
												<label for="title"><span class="required">*</span>${ktm:getText('page.gallery.head')
														}</label>
												<div class="controls">
										...
										<div class="control-group">
												<label for="content">${ktm:getText('page.gallery.desc')
														}</label>
												<div class="controls">
														<textarea  style="width: 400px;"  id="description" name="description">${bean.description}</textarea>
												</div>
										</div>
								</form>
								<br>
								<form id="fileupload" action="upload_image_gallery"
										class="span9" method="POST" enctype="multipart/form-data"
								>
										<input type="hidden" name="uniqueId" value="${bean.uniqueId}">
														...
														>
																<ktm:map_iterate name="bean" id="img" property="images">
																		<tr class="template-download fade in">
																				<td class="preview"><a
																						href="upload_image_gallery?getfile=${img.name}"
																						title="${img.name}" rel="gallery"
																						download="${img.name}"
																				><img
																								src="upload_image_gallery?getthumb=${img.name}"
																						></a></td>
																				<td class="name"><a
																						href="upload_image_gallery?getfile=${img.name}"
																						title="${img.name}" rel="gallery"
																				...
																				<td class="delete">
																						<button class="btn btn-danger" data-type="GET"
																								data-url="upload_image_gallery?delfile=${img.name}"
																						>
								...
								<div class="span9">
										...
										<div class="button-group pull-right">
												...
												<a class="btn btn-warning cancel"
														href="CRUDGallery?method=list&module=gallery&pageNumber=${bean.pageNumber}"
		...
		<script>
			function validateForm() {
				...
				bValid = bValid && checkLength( title,
																				"${ktm:getText('page.login.input')}${ktm:getText('page.gallery.head') }",
																				3,
																				512 );
				if ( bValid ) {
					$( "#form-error" ).fadeOut();
